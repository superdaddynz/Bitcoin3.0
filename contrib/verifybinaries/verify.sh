...
###   bitcoin3core.org and bitcoin3.org and compares them.
...
WORKINGDIR="/tmp/bitcoin3_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bitcoin3core.org"
HOST2="https://bitcoin3.org"
BASEDIR="/bin/"
VERSIONPREFIX="bitcoin3-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitcoin3-',
...
if [ $? -ne 0 ]; then
   echo "bitcoin3.org failed to provide signature file, but bitcoin3core.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitcoin3.org and bitcoin3core.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Bitcoin3 Core binary release signing key installed?"
   fi
