...
const char * const BITCOIN3_CONF_FILENAME = "bitcoin3.conf";
const char * const BITCOIN3_PID_FILENAME = "bitcoin3d.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "bitcoin3d -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Bitcoin3
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Bitcoin3
    // Mac: ~/Library/Application Support/Bitcoin3
    // Unix: ~/.bitcoin3
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bitcoin3";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Bitcoin3";
#else
    // Unix
    return pathRet / ".bitcoin3";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", BITCOIN3_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", BITCOIN3_PID_FILENAME)));
}
